                                        ...
                                        <div class="table-responsive">
                                            <table id="zero-configuration" class="display table nowrap table-striped table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                    ...
                                                    $get_user = $this->db->get('unmer_calon_mahasiswa.personal_data')->result();
...
function get_detail (id) {  
    window.location = ('<?= base_url() ?>admin/calon_mahasiswa/detail/'+id)
}
function get_edit (id) {
    window.location = ('<?= base_url() ?>admin/calon_mahasiswa/edit/'+id) 
}
</script>
<script>
...
function act_delete(id){
    where_value = id;
    where_field = 'id';
    table_name  = 'unmer_calon_mahasiswa.personal_data';
    $('.loading').show();
    $.ajax({
        type :"post",  
        url : "<?php echo base_url('admin/calon_mahasiswa/delete_data') ?>",  
        cache :false,
